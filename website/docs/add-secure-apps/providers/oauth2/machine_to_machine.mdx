...
## authentik-issued Tokens:ak-version[2026.2]

Token introspection of tokens issued by other OAuth2 providers is disabled by default. To allow this, you can configure which provider's
tokens can be used at the introspection endpoint.

You can do this in the "Cross-Provider Token Auth" section in the provider configuration.
...
You are only able to introspect a token:
 - from the same provider that was used to authenticate, or
 - from a provider listed in "Cross-Provider Token Auth" in the provider configuration, or
 - you must exchange the token for a token from the provider as described above
